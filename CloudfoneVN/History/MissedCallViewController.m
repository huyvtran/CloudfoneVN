...
- (void)showContentWithCurrentMessage {
    _lbNoCalls.text = [appDelegate.localization localizedStringForKey:@"No missed call in your history"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (aCall.duration < 60) {
        cell.lbDuration.text = SFM(@"%ld %@", aCall.duration, [appDelegate.localization localizedStringForKey:@"sec"]);
    }else{
        cell.lbDuration.text = SFM(@"%ld s", aCall.duration);
    }
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    descLabel.font = appDelegate.fontLarge;
