...
- (id)initWithState: (BOOL)state frame: (CGRect)frame
{
    ...
    if (self) {
        ...
        if (state) {
            ...
            lbState.text = [[[AppDelegate sharedInstance].localization localizedStringForKey:@"ON"] uppercaseString];
            ...
            lbState.text = [[[AppDelegate sharedInstance].localization localizedStringForKey:@"OFF"] uppercaseString];
...
- (void)tapToChangeValue {
    ...
    if (!networkReady) {
        [[AppDelegate sharedInstance].window makeToast:[[AppDelegate sharedInstance].localization localizedStringForKey:@"Please check your internet connection!"] duration:2.0 position:CSToastPositionCenter];
        return;
    }
